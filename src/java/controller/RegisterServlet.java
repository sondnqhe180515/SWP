    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (userDAO.isEmailExists(email)) {
                errorMessage = "Email \"" + email + "\" đã tồn tại. Vui lòng chọn email khác hoặc <a href='login.jsp'>đăng nhập</a>.";
                forwardError(fullName, passwordHash, repassword, email, phoneNumber, address, dobDay, dobMonth, dobYear, gender, errorMessage, request, response);
                return;
            }
            ...
            user.setPasswordHash(passwordHash); // In production, hash the password before storing!
            user.setEmail(email);
            ...
            errorMessage = "Lỗi cơ sở dữ liệu: " + ex.getMessage() + ". Vui lòng thử lại sau.";
            forwardError(fullName, passwordHash, repassword, email, phoneNumber, address, dobDay, dobMonth, dobYear, gender, errorMessage, request, response);
        }
    }
